...
- **Exit code is the truth.** A failed `render`, `eval`, `run`, `tokens`, `motion` or
  ...
  error-severity issue (warnings alone stay 0) — the same rule as `check`. An unresolved
  `var:` in `render` is a warning with a grey placeholder; `--strict-vars` makes it exit 1.
