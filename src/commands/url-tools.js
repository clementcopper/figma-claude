...
/**
 * argv for `npx capture-website-cli` — an array, never a shell string. The
 * URL is whatever the user typed; joined into one string it once ended the
 * quotes and ran the rest as a shell command.
 */
export function captureWebsiteArgs(url, outputFile, options) {
  const args = [
    '--yes', 'capture-website-cli', url,
    `--output=${outputFile}`,
    `--width=${options.width}`,
...
program
  ...
  .action(async (url, options) => {
    ...
    try {
      ...
      // Take screenshot (argv, no shell)
      execFileSync('npx', captureWebsiteArgs(url, tempFile, options), { stdio: 'ignore', timeout: 60000 });
