...
enum PanelConfigTests {
    ...
    // testAddsTheSessionNameForClaudeOnly
    do {
        var config = PanelConfig()
        Checks.expect(panelArguments(config: config, sessionName: "figma-claude:X"),
                       ["-n", "figma-claude:X"])
    ...
    do {
        var config = PanelConfig()
        config.args = ["-n", "mine"]
        Checks.expect(panelArguments(config: config, sessionName: "figma-claude:X"), ["-n", "mine"])
    }
