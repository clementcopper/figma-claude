...
- **Ein `.app`-Bundle nicht ersetzen, solange es läuft.** `install:app` löschte `/Applications/FigmaClaude.app` ungefragt; das Fenster lebte weiter, aber jeder neue Tab starb sofort mit `[Process exited with code 1]` ohne Ausgabe. Welche Datei genau fehlt, ist ungeklärt — `spawn-helper` von node-pty zu entfernen erzeugt eine andere Meldung (`posix_spawnp failed`). Der Installer beendet die App jetzt zuerst.
