...
### Benchmark the mode you think you are in (2026-09-11)

Measured all three transports (`npm run bench:transport`). Headline: the per-call round-trip is identical (~2-3 ms) in Yolo, Pipe and Safe — the daemon's HTTP hop dominates and the plugin's two extra postMessage hops cost nothing for a small eval. Safe's real cost is payload: a 100 KB byte array (an export's shape) is JSON-serialised three times (plugin main thread, UI iframe, daemon) and runs ~4x slower, growing with payload not call count. Pipe == Yolo speed, so Pipe wins outright. The old "~10x faster" was wrong both ways. Two gotchas the run surfaced: the export case is document-dependent (an empty active page returns null in 3 ms and looks fast), so add document-independent cases (`1+1`, a fixed byte array) for a fair cross-mode compare; and an `auto` daemon prefers a connected plugin, so with the Safe plugin still open `connect --patch` measured the plugin path, not CDP — force `DAEMON_MODE=cdp` to measure Yolo. Same evening, at 25 MB instead of 100 KB, the picture flipped: the pipe codec's quadratic string scan made Pipe 3x *slower* than Safe on an 8.6 MB export (65.6 s vs 21.6 s); fixed, Pipe is 13.6 s. A payload benchmark needs one case at the size users actually move (a full-page PNG), not only a 100 KB stand-in.
...
- **A cleanup rule fires in the path that only wanted to replace.** `closeTab` takes the window down when the last tab goes — right for closing, wrong inside the working-directory picker, which closed the only tab before opening its replacement. `applicationShouldTerminateAfterLastWindowClosed` then ended the app before the replacement line ran. It worked with two tabs open, which is why nobody saw it. Where a function carries a side effect for the edge case, check every caller that walks through that edge on purpose — and in a replace, open the new one first.
- **Finding a command on the PATH is not the same as being able to run it.** The About dialog resolved `figma-cli` on the login PATH but spawned it with the app's own environment. The installed `figma-cli` is a shim that runs `exec node …`, so from a Dock launch (`PATH=/usr/bin:/bin:/usr/sbin:/sbin`) the child died with `exec: node: not found`, exit 127 — and the dialog said `figma-cli —`. Any spawn of the CLI has to hand the resolved PATH to the child; `runCli` in `FigmaActions.swift` already does, which is why nothing else was affected.
- **A probe that stubs the thing under test proves nothing.** `--render-about` passed a fixed `2.1.2` to keep the PNG reproducible, so the version lookup was never exercised and the dialog shipped broken with every check green. Daniel found it by opening the app. Where a probe stubs an input, there needs to be a second, non-visual probe (`--print-about`) that runs the real path.
- **`cacheDisplay` gives you the view, not the window.** An About panel captured that way came out with every pixel outside the glyphs at `a0.00` — the window paints the background, the content view does not. In dark mode that is white text on transparency, which reads as a blank PNG. `view.wantsLayer = true` plus a layer background colour, set inside `effectiveAppearance.performAsCurrentDrawingAppearance`, puts the ground inside the captured hierarchy.
- **`NSImage.lockFocus` throws away the appearance context.** A dynamic colour set inside it resolves light no matter what the window is — which is why the first fix (composite the capture over `windowBackgroundColor`) still produced a white page. Pin such a colour to a concrete one *before* locking focus, or don't route through an NSImage at all.
- **A blank render is a measurement, not a look.** Three guesses at the cause were wrong; reading the actual pixels (`NSBitmapImageRep.colorAt`) answered it in one run. Hand-rolled PNG parsing does not count — 16-bit samples and per-row filters made the first numbers meaningless.
- **Anything the toolbar shows per tab belongs in `show(_:)`.** The folder button was set at launch and when picking a directory, never on a tab switch, so every tab wore the last directory chosen anywhere. Four paths bring a tab to the front — `activate`, `cycleTab`, `closeTab`, `respawn` — and all of them go through `show`; the status line was already there, with a comment describing this exact failure for the other bar.
- **Finder reads an app's icon through LaunchServices and keeps what it read.** Rebuilding into the same path left the generic document icon in `build/` while the Dock — which asks the running process — showed the real one. `touch` the bundle and `lsregister -f` it at the end of `make-app.sh`. The icns and `CFBundleIconFile` were correct the whole time, which is what the Dock proved.
...
- **A `timeout` parameter nobody reads is a hang with a name.** `runCli` and `LoginShellPath.resolve` both took one and both blocked in `readDataToEndOfFile` until the child closed the pipe; a hung `connect` kept `figmaBusy` for the life of the window. Grep every `timeout:` for the line that enforces it — `readOutput(_:from:timeout:)` is the one place now.
...
### Erster Build auf Apple Silicon: unsichtbare Bänder, blinder Pipe-Daemon (2026-09-16)

- **`dirtyRect` in `draw(_:)` zu füllen ist seit dem macOS-14-SDK ein Übermalen der Nachbarn.** Auf dem M2 (macOS 26.3.1, SDK 26.5) fehlten Toolbar und Tab-Strip komplett, Klicks funktionierten; auf dem Intel-Mac (älteres SDK) war alles da. Screenshot: die Bänder lagen exakt richtig, auch die zwei Trennlinien des Content-Views fehlten — alles, was in der Z-Reihenfolge *unter* dem Terminal-Container liegt. Ein einziger `dirtyRect.fill()` im Code (`TerminalColumn.draw`). Isoliert gemessen (`clip.swift`): `clipsToBounds` ist `false`, das Band unter dem Füller wird weiß (1.00 1.00 1.00); mit `bounds.intersection(dirtyRect)` bleibt es rot. `--render-chrome` misst jetzt die Trennlinien gegen die Spaltenfüllung — alter Code FAIL (1.00 vs 1.00), neuer ok (0.92 vs 1.00). Eine Farbmessung, die im Erfolgsfall und im Fehlerfall dasselbe sagt (Toolbar-Weiß gegen Terminal-Schwarz), war der erste Versuch und wurde verworfen.
- **`pages[0]` ist keine Datei-Wahl.** Figma stellt seine Tabs beim Start wieder her, ohne die Dokumente zu laden; ein solcher Tab hat keinen `figma`-Kontext. Der Pipe-Daemon hing stundenlang an Tab 1 (Bosch, ungeladen) mit „Could not find Figma execution context", der geladene m2trust stand an Position 2. Der Fehler wurde in `pipeConnectLoop` verschluckt (`catch { /* not yet */ }`), `/health` sagte nur `cdp:false` — das Panel zeigte „connecting…" ohne Ende. Jetzt: jeden Design-Tab probieren (0,5 s pro Fehlversuch), Grund einmal loggen und in `/health.pipeError` melden.
- **Ein Pin, der an vier Stellen nicht durchgereicht wird, existiert nicht.** `panel.json` hatte `figmaFile: "m2trust"`, aber: Panel-`connect()` ohne Env, `daemon restart` postete `/handoff` ohne `file`, der Nachfolger erbte die Env des *alten* Daemons, und `daemonPinMismatch` verglich bei `file: null` gar nicht. Erst `ps eww <daemon-pid> | grep FIGMA_FILE` zeigte das Loch; die Panel-Dateiliste war leer, weil sie im Pipe Mode `:9222/json` fragte, wo kein Port ist. Wo ein Wert über Prozessgrenzen wandert, jede Grenze einzeln prüfen — am laufenden Prozess, nicht im Code.
- **Ein Setup-Script, das nach seinem eigenen Namen sucht, findet die fremde Installation nicht.** Schritt 6 prüfte `claude mcp list | grep Framelink_Figma_MCP`; auf diesem Mac hieß der Eintrag `framelink` (Projekt-Scope, `~/Website`). Folge: keine Adoption des Keys, stattdessen die Frage nach einem Token, den Figma nur einmal zeigt. Erkennen am Kommando (`figma-developer-mcp` in den args), nicht am Namen. Dazu die Entscheidung: „global, aber standardmäßig aus" gibt es in Claude Code nicht als Schalter — User-Scope ist überall an, `/mcp` schaltet pro Projekt ab. Was es gibt: `--mcp-config <datei>` pro Prozess, ohne `--strict-mcp-config` additiv. Also Datei statt `~/.claude.json`-Eintrag, das Panel reicht sie pro Tab durch.
- **Ein flackernder Test ist ein Befund, kein Rauschen.** Der 413-Fall in `tests/daemon-live.test.js` fiel dreimal um (16.09. einmal von sieben Läufen, 18.09. zweimal von drei) und war allein nie zu reproduzieren — allein lief er ohne Last. Dahinter zwei Hälften: der Daemon rief `req.destroy()` direkt nach `res.end()` und riss damit den Socket ab, über den die Antwort noch ging (der Client sah `ECONNRESET` statt 413); und der Test verlangte einen Status, den HTTP hier nicht zusichern kann — wer einen 70-MB-Upload mittendrin abweist, schneidet dem Client die Schreibseite ab, und undici wirft dann `fetch failed`, statt die vielleicht schon empfangene 413 herauszugeben. Gefixt beides: erst spülen, dann schließen; und der Test prüft, was gilt (abgewiesen, Daemon lebt). Vier volle Läufe hintereinander grün.
- **Ein Health-Check, der in jedem Kontext gelingt, prüft nichts.** `cdpClient.eval('1')` war grün, während `typeof figma` im selben Tab `undefined` ergab — Figma hatte sein Plugin-Realm bei offenem Dokument abgebaut (16.09., ~14:40, kein Renderer-Crash: alle Helper-Prozesse von 10:15, keine Crash-Reports). Die Session sah zwei Stunden normale Arbeit, dann `getNodeByIdAsync of undefined` bei jedem Befehl und `status: Connected`. Der Probe muss die Frage stellen, an der die Befehle scheitern: `typeof figma`. Und ein Kontext ohne `figma` ist ein Verbindungsfehler, kein Code-Fehler — Client fallen lassen, neu anbinden. Was das Realm zurückbringt: Tab schließen und neu öffnen; `daemon restart` (gleiche Targets) und `connect` (reuse-pipe) nicht.
- **„Attempt 1 failed" liest jeder als „Attempt 2 folgt".** Die Panel-Session meldete einen Daemon-Retry mit doppelten Mutationen; das Log zeigte einen einzigen Versuch und danach „not retrying" — die Dopplung kam aus dem eigenen Neustart eines halb gelaufenen Scripts. Das Wort war trotzdem falsch, und die Lücke dahinter echt: Der Schutz vor einem Retry nach einem Figma-Fehler war nur der Health-Probe; ein Probe-Timeout bei beschäftigtem Figma hätte das Script erneut laufen lassen. Eine Exception aus Figma (`exceptionDetails`) heißt „der Code lief" — nie wiederholen, unabhängig von jeder Probe.
- **Der Gegentest muss die Hypothese treffen können.** „Klick den m2trust-Tab, wird es grün?" — Daniels Nein hätte die Diagnose gekippt, aber der Daemon konnte m2trust gar nicht versuchen (Pin fehlte). Erst prüfen, ob der Test überhaupt unterscheiden kann, dann fragen.
...
- **Guessing which wrapper compiles for `eval`.** Three regexes looking for `return` missed common shapes. `src/lib/eval-wrap.js` now asks the engine (`new Function(src)`) instead of guessing. The Safe Mode plugin kept its own `lastIndexOf(';')` guess for eight more weeks (until 2026-09-11) because nothing ran `plugin/code.js` in Node: four ordinary shapes were a SyntaxError in Safe Mode only, and the same review found `--timeout` never crossing the UI iframe and `throw 'text'` arriving as a silent success. `tests/plugin-executor.test.js` now loads the plugin with a stubbed `figma` and compares it against `eval-wrap.js` on every shape. A second implementation of a decision needs the same test as the first, or it drifts unseen. Same day, same pattern one layer up: `eval` classified daemon failures by message text beside `shouldFallBackToDirect`, and a reported 2 s timeout re-ran the code through the sync path (exit 0, "no value", 8 s); the live Safe Mode check found it, the 1006 green tests had not.
