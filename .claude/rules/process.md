...
- **A flag's "required" comment is a claim.** `--remote-allow-origins=*` was "required for Chrome 111+" and let every tab at the debug socket; probe the real client, a forged one and the thing the flag should keep out before keeping it.
- **A test that asserts more than the protocol can promise is the flaky one.** The 413 case asserted a status for an upload the daemon cuts off mid-stream; undici then throws `fetch failed` instead. It failed only under full-suite load — three sightings before anyone looked. Assert what is guaranteed (refused, still serving), and fix the real half: flush the answer before destroying the socket.
- **Test a signed app's launch through `open` with the quarantine xattr, on a copy.** Direct spawn bypasses LaunchServices and no Gatekeeper check runs; a modified bundle with its original signature is "damaged", so the ad-hoc re-sign after the asar patch stays.
