...
- **Never re-run code Figma raised an exception in** (`fromFigma` on the error, `retryVerdict` in `src/lib/exec-retry.js`): it ran up to the throw, a retry doubles every mutation before it. Only transport faults reconnect and retry.
- **A check a mode can never pass must not decide.** `FigmaClient.isConnected()` probes the debug port; Pipe and Safe never open one, so it was a guaranteed false and one transient no from `/health` ended the command. Portless modes ask the daemon twice (`/health/force`, `src/lib/connection-gate.js`); a negative health verdict is cached 2 s, not 30.
- **A prop that is accepted and never read is a silent lie.** `position` passed `warnUnknownProps` for a root `<Frame>` and was dropped, so an overlay joined the auto-layout flow. Either read it or warn — `tests/render-root-absolute.test.js`.
- **One generator for a parent lookup** (`src/lib/parent-snippet.js`): `render`, `instantiate` and `duplicate` place nodes, and only render forgot `loadAllPagesAsync`. A command that places a node names the page it landed on.
- **The CDP health probe asks `typeof figma`, never `1`** (`src/lib/cdp-health.js`): `1` succeeds in any context, and Figma can drop its plugin realm with the tab still open. A context without `figma` is a connection fault — release the client, re-attach.
- **Detect an installed MCP server by its command (`figma-developer-mcp`), never by the name we would give it;** secrets go into the config file's `env`, not argv. "On only in the panel" is `--mcp-config <file>` per tab (`panelArguments`), not a user-scope entry — Claude Code has no default-off for those.
- **A restored Figma tab has no `figma` context; never bind Pipe Mode to `pages[0]`.** `connectViaPipe` tries every design target (`pipeCandidates`), the pin travels through `/handoff` (`successorEnv`), and a swallowed connect error is a status that lies — log it once, report it in `/health.pipeError`.
