...
Useful in a report: what you did, what happened, which version (`figma-cli --version`), your OS, and which connection mode you were in (Pipe, Safe, Yolo or Browser).
...
Security review of figma-cli is mostly a question of what the four connection modes touch, so here it is in full. This is also the section to point at when a company asks before approving the tool. **Pipe Mode is the default on macOS and Linux** and is the least invasive; the others are opt-in.

### Pipe Mode (`figma-cli connect`, the default on macOS and Linux)

- **Patches nothing and opens no port.** The daemon launches Figma itself with `--remote-debugging-pipe` and speaks the Chrome DevTools Protocol over that pipe — file descriptors 3 and 4 of the Figma process it started, not a TCP socket. Figma does not strip that flag (unlike `--remote-debugging-port`), so no `app.asar` edit is needed, no `127.0.0.1:9222` is opened, and Figma keeps its Developer ID signature. None of Yolo Mode's compliance cost.
- The pipe is private to the daemon-and-Figma process pair. Unlike a CDP port, no other local process can reach it. `daemon restart` hands the pipe to the successor daemon (`POST /handoff`) so Figma is not dropped.
- Needs no macOS "App Management" permission — nothing is modified. The one constraint: the daemon must launch Figma itself; it cannot attach to an already-running Figma, so switching to Pipe Mode restarts it. `figjam` canvas commands need a port and are not available here.
- The daemon still runs arbitrary Plugin API code in the open file, same as every mode — see [The local daemon](#the-local-daemon).
...
### Yolo Mode (`figma-cli connect --patch`)

The old default, now opt-in via `--patch`; Pipe Mode reaches Figma the same way (CDP) without any of the following costs. Yolo still earns its place for a Figma that is already running (Pipe must launch its own), for the `figjam` canvas commands (they need a port), and on Windows (the pipe path is unverified there).
...
Out of scope: vulnerabilities in Figma Desktop itself (report those to Figma), and the inherent fact that Yolo and Browser mode open an unauthenticated local CDP port, which is documented above and is how the Chrome DevTools Protocol works. Pipe Mode (the default) opens no port, and Safe Mode none either.
